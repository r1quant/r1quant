Algumas recomendações organizadas por nível e foco:

**Fundamentos de econometria e séries temporais financeiras** — antes de mergulhar em regimes, é essencial ter uma base sólida. O livro de Ruey Tsay, Analysis of Financial Time Series, cobre modelos GARCH, volatilidade e dependência temporal de forma rigorosa. James Hamilton, Time Series Analysis, é a referência clássica e inclui o capítulo seminal sobre modelos de Markov-Switching, que são a espinha dorsal da detecção de regimes.

**Modelos de regime propriamente ditos** — o próprio Hamilton é o ponto de partida, já que ele praticamente fundou a literatura moderna de Markov regime-switching models. Para uma abordagem mais aplicada e voltada a asset management, o livro Asset Management: A Systematic Approach to Factor Investing de Andrew Ang dedica uma parte importante a como regimes (bull/bear, alta/baixa volatilidade) afetam prêmios de risco e alocação.

**Trading quantitativo e aplicação prática** — Ernest Chan tem dois livros muito acessíveis: Quantitative Trading e Algorithmic Trading, que cobrem estratégias sistemáticas com código. Marcos López de Prado, Advances in Financial Machine Learning, é excelente para quem quer aplicar técnicas de machine learning à detecção de regimes, com discussões sobre structural breaks, meta-labeling e feature importance.

**Machine learning e métodos estatísticos avançados** — Kevin Murphy, Machine Learning: A Probabilistic Perspective, tem capítulos excelentes sobre Hidden Markov Models (HMMs), que são diretamente aplicáveis à modelagem de regimes. Para uma abordagem bayesiana, Bayesian Data Analysis de Gelman et al. dá a base para modelos hierárquicos e inferência de estados latentes.

**Leitura complementar** — vale acompanhar papers acadêmicos diretamente. Autores como Ang & Bekaert (regimes e alocação), Bulla & Bulla (HMMs em finanças) e Guidolin & Timmermann (regime-switching em portfólios) são referências frequentes.

**Machine learning aplicado a regimes:**

- López de Prado, M. — _Advances in Financial Machine Learning_: capítulos sobre structural breaks e meta-labeling são diretamente aplicáveis à detecção de regime com ML. O conceito de "triple barrier labeling" pode ser adaptado para gerar labels de regime de forma menos arbitrária.

**Reinforcement learning em finanças:**

- Zhang, Y., Zohren, S. & Roberts, S. (2020). "Deep Reinforcement Learning for Trading." _Journal of Financial Data Science_, 2(2). — Framework sólido de DRL para trading; discussão honesta de limitações.

**Regime-switching aplicado a crypto:**

- A literatura específica de regime-switching para Bitcoin é ainda escassa mas crescente. O Fear & Greed Index e funding rates como features de regime são contribuições mais da prática (prop trading, DeFi) do que da academia formal. Monitorar publicações no _Journal of Financial Data Science_ e _Quantitative Finance_ para papers emergentes sobre crypto regime detection.
...
**Estrutura formal:** o modelo assume K regimes discretos. Em cada regime k, os retornos seguem uma distribuição com parâmetros específicos (μ*k, σ_k). A transição entre regimes é governada por uma matriz de probabilidades de transição P, onde p_ij = P(S_t = j | S*{t-1} = i). A estimação é feita via máxima verossimilhança usando o filtro de Hamilton, que calcula a probabilidade filtrada P(S_t = k | Y_1, ..., Y_t) a cada período.
